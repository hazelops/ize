...
func TestManager_Destroy(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockECS: func(m *mocks.MockECSAPI) {
				...
				m.EXPECT().DeregisterTaskDefinition(gomock.Any()).Return(nil, awserr.New(ecs.ErrCodeInvalidParameterException, "", nil)).Times(1)
			},
			wantErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config.InitConfig()
			tt.fields.Project.AWSClient = config.NewAWSClient(config.WithECSClient(mockECSAPI))
			err = tt.fields.Project.GetTestConfig()
			if err != nil {
				t.Error(err)
				return
